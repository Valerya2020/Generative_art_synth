...
import { model } from '../prototypes/prototype_69/model'

import {
  setBackgroundColorClass,
  getLargeCircleStore,
  setLargeCircleStore
} from '../prototypes/prototype_69/store'
...
function generateLargeCircle() {
  return new Promise((resolve, reject) => {
    ...
    (
      ...
      const sizeInPixels =
        getRandomArbitrary(sizeParams.from, sizeParams.to) + 'px'
      ...
      if (vSide === 'top') {
        circle.style.top =
          getRandomArbitrary(vSideParams.from, vSideParams.to) + 'px'
      }
      ...
      if (hSide === 'left') {
        circle.style.left =
          getRandomArbitrary(hSideParams.from, hSideParams.to) + 'px'
      }
...
document.addEventListener('DOMContentLoaded', () => {
  container = document.getElementById('prototype_69')
  generateStory()
})
